...
#include "DRIVERS_PROYECTO/include/GPIO_DRIVER.h"
...
#define S_IN1 27
#define S_OUT2 14
#define button_isr3 12
#define button_isr4 13

//ventilador
#define button_isr5 4
#define button_isr6 5
#define FAN 16
...
void isr_handler1(void *args);
void isr_handler2(void *args);
void isr_handler3(void *args);
void isr_handler4(void *args);
void isr_handler5(void *args);
void isr_handler6(void *args);

//////////////////////////////////////////////////////////////////////////////////////

esp_err_t init_irs_botones(void)
{
    gpio_config_t pGPIOConfig1;
    pGPIOConfig1.pin_bit_mask = (1ULL << S_IN1);
    pGPIOConfig1.mode = GPIO_MODE_DEF_INPUT;
    pGPIOConfig1.pull_up_en = GPIO_PULLUP_DISABLE;
    pGPIOConfig1.pull_down_en = GPIO_PULLDOWN_DISABLE;
    pGPIOConfig1.intr_type = GPIO_INTR_NEGEDGE;

    gpio_config(&pGPIOConfig1);

    gpio_install_isr_service(0);                     //instalar globalmente las interrupciones
    gpio_isr_handler_add(S_IN1, isr_handler1, NULL);  //agregar pin a las interrupiones (isr1_handler) funcion que se ejecuta cuando se habilita la interrupcion

    gpio_config_t pGPIOConfig2;
    pGPIOConfig2.pin_bit_mask = (1ULL << S_OUT2);
    pGPIOConfig2.mode = GPIO_MODE_DEF_INPUT;
    pGPIOConfig2.pull_up_en = GPIO_PULLUP_DISABLE;
    ...
    gpio_isr_handler_add(S_OUT2, isr_handler2, NULL);

    gpio_config_t pGPIOConfig3;
    pGPIOConfig3.pin_bit_mask = (1ULL << button_isr3);
    pGPIOConfig3.mode = GPIO_MODE_DEF_INPUT;
    pGPIOConfig3.pull_up_en = GPIO_PULLUP_DISABLE;
    pGPIOConfig3.pull_down_en = GPIO_PULLDOWN_DISABLE;
    pGPIOConfig3.intr_type = GPIO_INTR_NEGEDGE;

    gpio_config(&pGPIOConfig3);

    gpio_install_isr_service(0);
    gpio_isr_handler_add(button_isr3, isr_handler3, NULL);

    gpio_config_t pGPIOConfig4;
    pGPIOConfig4.pin_bit_mask = (1ULL << button_isr4);
    pGPIOConfig4.mode = GPIO_MODE_DEF_INPUT;
    pGPIOConfig4.pull_up_en = GPIO_PULLUP_DISABLE;
    pGPIOConfig4.pull_down_en = GPIO_PULLDOWN_DISABLE;
    pGPIOConfig4.intr_type = GPIO_INTR_NEGEDGE;

    gpio_config(&pGPIOConfig4);

    gpio_install_isr_service(0);
    gpio_isr_handler_add(button_isr4, isr_handler4, NULL);

    gpio_config_t pGPIOConfig5;
    pGPIOConfig5.pin_bit_mask = (1ULL << button_isr5);
    pGPIOConfig5.mode = GPIO_MODE_DEF_INPUT;
    pGPIOConfig5.pull_up_en = GPIO_PULLUP_DISABLE;
    pGPIOConfig5.pull_down_en = GPIO_PULLDOWN_DISABLE;
    pGPIOConfig5.intr_type = GPIO_INTR_NEGEDGE;

    gpio_config(&pGPIOConfig5);

    gpio_install_isr_service(0);
    gpio_isr_handler_add(button_isr5, isr_handler5, NULL);

    gpio_config_t pGPIOConfig6;
    pGPIOConfig6.pin_bit_mask = (1ULL << button_isr6);
    pGPIOConfig6.mode = GPIO_MODE_DEF_INPUT;
    pGPIOConfig6.pull_up_en = GPIO_PULLUP_DISABLE;
    pGPIOConfig6.pull_down_en = GPIO_PULLDOWN_DISABLE;
    pGPIOConfig6.intr_type = GPIO_INTR_NEGEDGE;
    

    gpio_config(&pGPIOConfig6);
    gpio_install_isr_service(0);
    gpio_isr_handler_add(button_isr6, isr_handler6, NULL);
    
    ESP_LOGI(tag, "inicialización de ISR completada");
    return ESP_OK;
}
...
esp_err_t Alarma1(){
                       ...
                       gpio_hal_set_level(gpio_context.gpio_hal, ledA, 1);
                       // gpio_set_level(ledAB, 0);
                      
                        gpio_hal_set_level(gpio_context.gpio_hal, ledAB, 0);
                       return ESP_OK;
}
esp_err_t Alarma2(){
    
                       // gpio_set_level(ledA, 1);
                       gpio_hal_set_level(gpio_context.gpio_hal, ledA, 0);
                       // gpio_set_level(ledAB, 0);
                      
                        gpio_hal_set_level(gpio_context.gpio_hal, ledAB, 1);
                       return ESP_OK;
}
esp_err_t Alarma_apagada(){
...
////////////////////////////////////////////////////////////
esp_err_t leds_apagados(){ //para el reset
                      
                       gpio_hal_set_level(gpio_context.gpio_hal, ledA, 0);
                        gpio_hal_set_level(gpio_context.gpio_hal, ledAB, 0);
                           gpio_hal_set_level(gpio_context.gpio_hal, FAN, 0);
                        gpio_hal_set_level(gpio_context.gpio_hal, DOOR1, 0);
                         gpio_hal_set_level(gpio_context.gpio_hal, DOOR2, 0);
                           gpio_hal_set_level(gpio_context.gpio_hal, ledB, 0);
                       return ESP_OK;
}
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//BOTONES DESHABILITADOS MENOS EL 3
// gpio_set_intr_type(S_IN1, GPIO_INTR_DISABLE);

esp_err_t gpio_set_intr_type1() //interrupion deshabilitada para el pin 27
{
   
    portENTER_CRITICAL(&gpio_context.gpio_spinlock);
    gpio_hal_set_intr_type(gpio_context.gpio_hal, 27, GPIO_INTR_DISABLE);
    if (GPIO_INTR_DISABLE == GPIO_INTR_POSEDGE || GPIO_INTR_DISABLE == GPIO_INTR_NEGEDGE || GPIO_INTR_DISABLE == GPIO_INTR_ANYEDGE) {
        gpio_context.isr_clr_on_entry_mask |= (1ULL << (27));
    } else {
        gpio_context.isr_clr_on_entry_mask &= ~(1ULL << (27));
    }
    portEXIT_CRITICAL(&gpio_context.gpio_spinlock);
...
esp_err_t gpio_set_intr_type2()  //interrupcion deshabilitada para el pin 14
{
    
    portENTER_CRITICAL(&gpio_context.gpio_spinlock);
    gpio_hal_set_intr_type(gpio_context.gpio_hal, 14, GPIO_INTR_DISABLE);
    if (GPIO_INTR_DISABLE == GPIO_INTR_POSEDGE || GPIO_INTR_DISABLE == GPIO_INTR_NEGEDGE || GPIO_INTR_DISABLE == GPIO_INTR_ANYEDGE) {
        gpio_context.isr_clr_on_entry_mask |= (1ULL << (14));
    } else {
        gpio_context.isr_clr_on_entry_mask &= ~(1ULL << (14));
    }
    portEXIT_CRITICAL(&gpio_context.gpio_spinlock);
...
esp_err_t gpio_set_intr_type3() //interrupcion deshabilitada para el pin 13
{
    portENTER_CRITICAL(&gpio_context.gpio_spinlock);
    gpio_hal_set_intr_type(gpio_context.gpio_hal, 13, GPIO_INTR_DISABLE);
    if (GPIO_INTR_DISABLE == GPIO_INTR_POSEDGE || GPIO_INTR_DISABLE == GPIO_INTR_NEGEDGE || GPIO_INTR_DISABLE == GPIO_INTR_ANYEDGE) {
        gpio_context.isr_clr_on_entry_mask |= (1ULL << (13));
    } else {
        gpio_context.isr_clr_on_entry_mask &= ~(1ULL << (13));
    }
    portEXIT_CRITICAL(&gpio_context.gpio_spinlock);
...
esp_err_t gpio_set_intr_type4() //interrupcion deshabilitada para el pin 4
{
    portENTER_CRITICAL(&gpio_context.gpio_spinlock);
    gpio_hal_set_intr_type(gpio_context.gpio_hal, 4, GPIO_INTR_DISABLE);
    if (GPIO_INTR_DISABLE == GPIO_INTR_POSEDGE || GPIO_INTR_DISABLE == GPIO_INTR_NEGEDGE || GPIO_INTR_DISABLE == GPIO_INTR_ANYEDGE) {
        gpio_context.isr_clr_on_entry_mask |= (1ULL << (4));
    } else {
        gpio_context.isr_clr_on_entry_mask &= ~(1ULL << (4));
    }
    portEXIT_CRITICAL(&gpio_context.gpio_spinlock);
...
esp_err_t gpio_set_intr_type5() //interrupcion deshabilitada para el pin 5
{
    portENTER_CRITICAL(&gpio_context.gpio_spinlock);
    gpio_hal_set_intr_type(gpio_context.gpio_hal, 5, GPIO_INTR_DISABLE);
    if (GPIO_INTR_DISABLE == GPIO_INTR_POSEDGE || GPIO_INTR_DISABLE == GPIO_INTR_NEGEDGE || GPIO_INTR_DISABLE == GPIO_INTR_ANYEDGE) {
        gpio_context.isr_clr_on_entry_mask |= (1ULL << (5));
    } else {
        gpio_context.isr_clr_on_entry_mask &= ~(1ULL << (5));
    }
    portEXIT_CRITICAL(&gpio_context.gpio_spinlock);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
esp_err_t hab_gpio_set_intr_type1() //interrupion habilitada para el pin 27 en negativo
{
    portENTER_CRITICAL(&gpio_context.gpio_spinlock);
    gpio_hal_set_intr_type(gpio_context.gpio_hal, 27, GPIO_INTR_NEGEDGE);
    if (GPIO_INTR_NEGEDGE == GPIO_INTR_POSEDGE || GPIO_INTR_NEGEDGE == GPIO_INTR_NEGEDGE || GPIO_INTR_NEGEDGE == GPIO_INTR_ANYEDGE) {
        gpio_context.isr_clr_on_entry_mask |= (1ULL << (27));
    } else {
        gpio_context.isr_clr_on_entry_mask &= ~(1ULL << (27));
    }
...
esp_err_t hab_gpio_set_intr_type2()  //interrupcion habilitada para el pin 14
{
    portENTER_CRITICAL(&gpio_context.gpio_spinlock);
    gpio_hal_set_intr_type(gpio_context.gpio_hal, 14, GPIO_INTR_NEGEDGE);
    if (GPIO_INTR_NEGEDGE == GPIO_INTR_POSEDGE || GPIO_INTR_NEGEDGE == GPIO_INTR_NEGEDGE || GPIO_INTR_DISABLE == GPIO_INTR_ANYEDGE) {
        gpio_context.isr_clr_on_entry_mask |= (1ULL << (14));
    } else {
        gpio_context.isr_clr_on_entry_mask &= ~(1ULL << (14));
    }
...
esp_err_t hab_gpio_set_intr_type3() //interrupcion habilitada para el pin 13
{
    portENTER_CRITICAL(&gpio_context.gpio_spinlock);
    gpio_hal_set_intr_type(gpio_context.gpio_hal, 13, GPIO_INTR_NEGEDGE);
    if (GPIO_INTR_NEGEDGE == GPIO_INTR_POSEDGE || GPIO_INTR_NEGEDGE == GPIO_INTR_NEGEDGE || GPIO_INTR_NEGEDGE == GPIO_INTR_ANYEDGE) {
        gpio_context.isr_clr_on_entry_mask |= (1ULL << (13));
    } else {
        gpio_context.isr_clr_on_entry_mask &= ~(1ULL << (13));
...
esp_err_t hab_gpio_set_intr_type4() //interrupcion habilitada para el pin 4
{
...
esp_err_t hab_gpio_set_intr_type5() //interrupcion habilitada para el pin 5 en flanco de bajada
{
    portENTER_CRITICAL(&gpio_context.gpio_spinlock);
    gpio_hal_set_intr_type(gpio_context.gpio_hal, 5, GPIO_INTR_NEGEDGE);
    if (GPIO_INTR_NEGEDGE == GPIO_INTR_POSEDGE || GPIO_INTR_NEGEDGE == GPIO_INTR_NEGEDGE || GPIO_INTR_NEGEDGE == GPIO_INTR_ANYEDGE) {
        gpio_context.isr_clr_on_entry_mask |= (1ULL << (5));
    } else {
        gpio_context.isr_clr_on_entry_mask &= ~(1ULL << (5));
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
int gpio_nivel(gpio_num_t gpio_num) ///////////////////int gpio_get_level(gpio_num_t gpio_num) para los botones en los if
{
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
esp_err_t led_directions(){         //gpio_set_directions
    //1

    if ((GPIO_IS_VALID_OUTPUT_GPIO(15) != true) && (GPIO_MODE_OUTPUT & GPIO_MODE_DEF_OUTPUT)) {
        ESP_LOGE(GPIO_TAG, "io_num=%d can only be input", 15);
        return ESP_ERR_INVALID_ARG;
    }

    esp_err_t ret = ESP_OK;

    if (GPIO_MODE_OUTPUT & GPIO_MODE_DEF_INPUT) {
        gpio_input_enable(15);
    } else {
        gpio_input_disable(15);
    }

    if (GPIO_MODE_OUTPUT & GPIO_MODE_DEF_OUTPUT) {
        gpio_output_enable(15);
    } else {
        gpio_output_disable(15);
    }

    if (GPIO_MODE_OUTPUT & GPIO_MODE_DEF_OD) {
        gpio_od_enable(15);
    } else {
        gpio_od_disable(15);
    }
    ...
    if ((GPIO_IS_VALID_OUTPUT_GPIO(2) != true) && (GPIO_MODE_OUTPUT & GPIO_MODE_DEF_OUTPUT)) {
        ESP_LOGE(GPIO_TAG, "io_num=%d can only be input", 2);
        return ESP_ERR_INVALID_ARG;
    }
    ...
    if (GPIO_MODE_OUTPUT & GPIO_MODE_DEF_INPUT) {
        ...
        gpio_input_disable(2);
    }

    if (GPIO_MODE_OUTPUT & GPIO_MODE_DEF_OUTPUT) {
        gpio_output_enable(2);
    } else {
        gpio_output_disable(2);
    }
    ...
    if ((GPIO_IS_VALID_OUTPUT_GPIO(0) != true) && (GPIO_MODE_OUTPUT & GPIO_MODE_DEF_OUTPUT)) {
        ESP_LOGE(GPIO_TAG, "io_num=%d can only be input", 0);
        return ESP_ERR_INVALID_ARG;
    }
    ...
    if ((GPIO_IS_VALID_OUTPUT_GPIO(21) != true) && (GPIO_MODE_OUTPUT & GPIO_MODE_DEF_OUTPUT)) {
        ESP_LOGE(GPIO_TAG, "io_num=%d can only be input", 21);
        return ESP_ERR_INVALID_ARG;
    }    

    if (GPIO_MODE_OUTPUT & GPIO_MODE_DEF_INPUT) {
        gpio_input_enable(21);
    } else {
        gpio_input_disable(21);
    }

    if (GPIO_MODE_OUTPUT & GPIO_MODE_DEF_OUTPUT) {
        gpio_output_enable(21);
    } else {
        gpio_output_disable(21);
    }
    ...
    if ((GPIO_IS_VALID_OUTPUT_GPIO(22) != true) && (GPIO_MODE_OUTPUT & GPIO_MODE_DEF_OUTPUT)) {
        ESP_LOGE(GPIO_TAG, "io_num=%d can only be input", 22);
        return ESP_ERR_INVALID_ARG;
    }
    ...
    if (GPIO_MODE_OUTPUT & GPIO_MODE_DEF_INPUT) {
        gpio_input_enable(22);
    } else {
        gpio_input_disable(22);
    }

    if (GPIO_MODE_OUTPUT & GPIO_MODE_DEF_OUTPUT) {
        gpio_output_enable(22);
    } else {
        gpio_output_disable(22);
    }
    ...
    if ((GPIO_IS_VALID_OUTPUT_GPIO(16) != true) && (GPIO_MODE_OUTPUT & GPIO_MODE_DEF_OUTPUT)) {
        ESP_LOGE(GPIO_TAG, "io_num=%d can only be input", 16);
        return ESP_ERR_INVALID_ARG;
    }
    ...
    if (GPIO_MODE_OUTPUT & GPIO_MODE_DEF_INPUT) {
        gpio_input_enable(16);
    } else {
        gpio_input_disable(16);
    }

    if (GPIO_MODE_OUTPUT & GPIO_MODE_DEF_OUTPUT) {
        gpio_output_enable(16);
    } else {
        gpio_output_disable(16);
    }
...
//////////////////////////////////////////////////////////////////////////////////////////////////
esp_err_t reset_leds(){ //para la parte de gpio_reset_pin
//1
assert(GPIO_IS_VALID_GPIO(15));
    gpio_config_t cfg = {
